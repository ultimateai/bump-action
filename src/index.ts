...
const start = async () => {
  try {
    ...
    const commitMessage: CommitMessageQueryResponse = await octokit.graphql(
      commitMessageQuery,
      {
        ...repoDetails,
    ...
    const latestRelease: LatestReleaseQueryResponse = await octokit.graphql(
      lastReleaseQuery,
      {
        ...repoDetails,
      }
    ...
    const bumpType: Bump = determineBumpType(
      commitMessage.repository.pullRequest.mergeCommit,
      {
    ...
    let headerMessage =
      commitMessage.repository.pullRequest.mergeCommit.messageHeadline;
    let bodyMessage =
      commitMessage.repository.pullRequest.mergeCommit.messageBody;
    ...
    if (createRelease) {
      const releaseResult = await octokit.request(
      ...
      console.log(
        `Skipping GitHub release creation for tag: ${nextReleaseTag} (create_release=false)`
      );
    }
    if (core.getInput("update_file")) {
      console.log(
        "Input file to be modified is " + core.getInput("update_file")
      );
      if (core.getInput("update_file") == "package.json") {
        //Get input file
        const fileToUpdate = await octokit.request(
          `GET /repos/{owner}/{repo}/contents/${core.getInput("update_file")}`,
          {
            repo: repoDetails.repoName,
            owner: repoDetails.repoOwner,
            branch: targetBranch,
        ...
        const fileContent = JSON.parse(
          Base64.decode(fileToUpdate.data.content)
        );
        fileContent.version = nextReleaseTag;
        updatedFileContent = Base64.encode(
          JSON.stringify(fileContent, null, 4)
        );
        const updateFileResult = await octokit.request(
          `PUT /repos/{owner}/{repo}/contents/${core.getInput("update_file")}`,
          {
            repo: repoDetails.repoName,
            owner: repoDetails.repoOwner,
            message: `Automatic ${core.getInput(
              "update_file"
            )} bump to ${nextReleaseTag}`,
            branch: targetBranch,
            sha: fileSha,
        ...
        core.setFailed(
          "Your update_file does not exist or it's not supported."
        );
      }
    }
    if (core.getInput("changelog") && core.getInput("changelog") == "true") {
      ...
      const fileToUpdate = await octokit.request(
        `GET /repos/{owner}/{repo}/contents/${repoDetails.changelogFile}`,
        {
          repo: repoDetails.repoName,
          owner: repoDetails.repoOwner,
          branch: targetBranch,
      ...
      updatedFileContent = Base64.encode(
        changelogDate.toISOString().split("T")[0] +
          ", " +
          nextReleaseTag +
          "\n\n" +
      ...
      const changelogResult = await octokit.request(
        `PUT /repos/{owner}/{repo}/contents/${repoDetails.changelogFile}`,
        {
          repo: repoDetails.repoName,
          owner: repoDetails.repoOwner,
